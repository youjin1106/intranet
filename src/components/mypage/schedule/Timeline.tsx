...
const Timeline = () => {
	...
	const getSchedules = async (id: string) => {
		const response = await fetch(`http://localhost:3001/schedules/${id}`).then(res => res.json());
		return response;
	};
	...
	const getSchedule = async () => {
		...
		getData.schedule.map((timeRecord: JsonUserScheduleData) => {
			...
			const filteredTimeRecord: TimeRange = {
				x: timeRecord.date,
				y: [dayjs(`2024-01-31 ${startTime}`).valueOf(), dayjs(`2024-01-31 ${endTime}`).valueOf()],
			};
	...
	return (
		<div className="w-full h-[316px] bg-bg01 rounded p-[16px]">
			<div className="flex flex-row">
				<div className="text-titleMd text-gray00 font-bold m-2 ml-3 mt-2">근무 관리</div>
				<WorkingTimer />
			</div>
