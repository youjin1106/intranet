...
const createSelectedOptionAtom = (initialValue: string) =>
  atom<string>(initialValue);

const SelectBox: React.FC<SelectBoxProps> = ({ options, size }) => {
  const selectedOptionAtom = useMemo(
    () => createSelectedOptionAtom(options[0]),
    [options]
  );
  ...
  return (
    <div>
      <div
        ref={selectBoxRef}
        className={`custom-select selected-option ${
          size === "lg" ? "w-full" : "w-[120px]"
        } bg-bg01 text-md text-gray00 h-8 pl-4 pr-2 py-0.5 rounded border-2 border-solid border-border relative select-none hover:border-primary flex justify-between
       cursor-pointer`}
